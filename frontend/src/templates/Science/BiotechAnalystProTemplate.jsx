  ...
  const runDownloadProcess = async () => {
    try {
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `Biotech_Analyst_${data.lastName}_${cvNumber}.pdf`,
      ...
      console.error("Critical: Biotech Data Archiving failed", err);
    } finally {
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-100">
            <h3 className="text-xl font-bold text-gray-900 mb-2">Archive Research Asset?</h3>
            <p className="text-sm text-gray-600 mb-6 italic">This action will sync the current scientific profile with the central bioinformatics repository and generate a secure PDF document.</p>
            <div className="flex justify-end gap-3">
              ...
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-4 py-2 rounded-lg text-white font-bold transition-all shadow-md text-xs uppercase tracking-wider" style={{ backgroundColor: templateConfig.primaryColor }}>
                {isDownloading ? "Syncing..." : "Confirm Archive"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 text-slate-900">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl text-center border-t-8 border-[#008080]">
            <h3 className="text-2xl font-bold mb-2">Protocol Complete</h3>
            <p className="text-sm text-gray-600 mb-4 italic">Thescientific profile has been validated and synced with the Biotech repository.</p>
            <div className="bg-slate-50 border-2 border-dashed border-gray-200 p-5 rounded-xl mb-6">
               <div className="flex justify-center mb-2 text-[#008080]"><ShieldCheck size={32}/></div>
               <p className="text-[10px] text-gray-400 uppercase font-black tracking-[4px] mb-1">Validation ID</p>
               <p className="text-3xl font-black text-[#008080] font-mono tracking-tighter">{generatedCvNumber}</p>
            </div>
            <button onClick={() => setShowSuccessModal(false)} className="w-full py-3.5 rounded-lg text-white font-black uppercase tracking-widest shadow-lg hover:shadow-xl transition-all" style={{ backgroundColor: templateConfig.primaryColor }}>Acknowledge Protocol</button>
          </div>
        </div>
      )}
    </div>
