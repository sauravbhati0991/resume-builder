...
export default function CustomerSuccessLeadTemplate() {
  ...
  const saveDraft = () => {
    setIsSaving(true);
    setTimeout(() => {
      localStorage.setItem(`cs_lead_sync_${templateId}`, JSON.stringify(data));
      setIsSaving(false);
    }, 800);
  };
  ...
  const runDownloadProcess = async () => {
    try {
      ...
      // 2. High-Scale Rendering (Scale 3 + Elite Quality)
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `SUCCESS_LEAD_${cvId}.pdf`,
      ...
      // 3. FormData Archival (Dual Storage Handshake)
      ...
      // 4. Archive Success -> Final Download
      await worker.save();
      setShowSuccessModal(true);
    } catch (err) {
      console.error("Success Portfolio Archival Failure:", err);
    } finally {
  ...
  return (
    <div className="fixed inset-0 bg-slate-50 flex flex-col overflow-hidden font-sans text-slate-800 z-[60]">
      ...
      <div className="w-full bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center z-20 shadow-sm">
        ...
        <div className="flex items-center gap-3">
          <button onClick={saveDraft} disabled={isSaving} className="flex items-center gap-2 bg-white border border-slate-200 text-slate-500 px-4 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-slate-50 transition-all">
            {isSaving ? <Loader2 size={16} className="animate-spin" /> : <Database size={16} />} 
            Sync Draft
          </button>
          <button onClick={() => setShowReplaceModal(true)} disabled={isDownloading} className="flex items-center gap-2 bg-[#87CEEB] text-slate-900 px-6 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg shadow-sky-100 hover:opacity-90 transition-all active:scale-95 disabled:opacity-50">
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-8 shadow-2xl border-t-[8px] border-[#87CEEB]">
            ...
            <p className="text-sm text-slate-500 mb-8 leading-relaxed font-medium">This will archive your executive success metrics in the secure database and generate a high-scale certified document.</p>
