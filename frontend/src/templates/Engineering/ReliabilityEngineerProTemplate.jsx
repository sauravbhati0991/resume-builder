...
export default function ReliabilityEngineerProTemplate({
  ...
  return (
    <div className="min-h-screen w-full bg-slate-100 flex flex-col overflow-hidden font-sans text-slate-900">
        ...
        <div className="bg-white border border-slate-300 shadow-sm rounded-xl p-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
              ...
              >
                ...
                {generatedCvNumber || cvNumber ? "Update_System" : "Sync_System"}
              </button>
              <button
                onClick={() => handlePdfDownload(generatedCvNumber)}
                disabled={!generatedCvNumber}
                className={`inline-flex items-center text-sm font-bold h-9 px-4 rounded-md text-white shadow-lg hover:brightness-110 transition-all uppercase ${generatedCvNumber
      ...
      <div className="flex-1 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-4 w-full overflow-hidden min-h-0">
        <div className="grid lg:grid-cols-2 gap-6 h-full">
            ...
            <div className="h-full bg-slate-300/50 rounded-xl overflow-auto flex justify-center p-8 custom-scrollbar">
                <div id="resume-preview" ref={previewRef} style={{ width: '210mm', minHeight: '297mm', backgroundColor: 'white', boxShadow: '0 10px 40px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/90 backdrop-blur-md px-4 text-center text-slate-900 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl border-t-8 border-red-500">
            <div className="w-20 h-20 bg-red-50 text-red-600 rounded-full flex items-center justify-center mx-auto mb-6">
              <Activity size={48} />
            </div>
            <h3 className="text-2xl font-black mb-2 uppercase tracking-tight">System Status: Verified</h3>
            <p className="text-sm text-slate-500 mb-8 font-medium italic underline decoration-red-500/30 underline-offset-4 tracking-wide">
              Technical assets successfully serialized and archived to the primary directory.
            </p>
