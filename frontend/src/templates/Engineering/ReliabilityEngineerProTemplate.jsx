...
export default function ReliabilityEngineerProTemplate() {
  ...
  const saveResume = () => {
    setIsSaving(true);
    setTimeout(() => {
      localStorage.setItem(`reliability_sync_${templateId}`, JSON.stringify(data));
      setIsSaving(false);
    }, 800);
  };

  // MASTER LOGIC: The Reliability Handshake
  const runDownloadProcess = async () => {
    try {
      ...
      const res = await api.post("/resumes", {
        ...
        categoryName: "Reliability Engineering",
        resumeData: data
      });

      const cvNumber = res.data.cvNumber;

      // STEP 2: High-Quality PDF Render (Scale 3 + Scroll Fix)
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `RELIABILITY_ENG_${cvNumber}.pdf`,
      ...
      // STEP 4: Success Trigger
      await worker.save();
      setGeneratedCvNumber(cvNumber);
      setShowSuccessModal(true);
    } catch (err) {
      console.error("Critical Asset Failure during export:", err);
    } finally {
    ...
    <div className="min-h-screen w-full bg-slate-100 flex flex-col overflow-hidden font-sans text-slate-900">
            ...
            <div className="h-full overflow-y-auto pr-2 custom-scrollbar pb-20 space-y-6">
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <InputGroup label="Email Contact" value={data.email} onChange={(v)=>handleInputChange('email', v)}/>
                        <InputGroup label="Comm. Phone" value={data.phone} onChange={(v)=>handleInputChange('phone', v)}/>
                        <InputGroup label="Primary Base" value={data.location} onChange={(v)=>handleInputChange('location', v)} className="col-span-2"/>
                    </div>
                </div>

                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    <h3 className="text-sm font-black mb-4 uppercase tracking-widest text-slate-800">Analytical Profile</h3>
                    <textarea rows={4} value={data.summary} onChange={(e)=>handleInputChange('summary', e.target.value)} className="w-full border border-slate-300 rounded-md p-3 text-sm focus:ring-2 focus:ring-slate-500 outline-none leading-relaxed bg-slate-50 font-sans"/>
                </div>
                     ...
                     {data.experience.map((exp, i) => (
                        <div key={i} className="mb-4 p-4 border border-slate-100 rounded-lg bg-slate-50 relative group">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <textarea rows={3} placeholder="Quantify reliability gains (MTBF, MTTR, Savings)..." value={exp.description} onChange={(e)=>handleArrayChange(i,'description',e.target.value,'experience')} className="col-span-2 border border-slate-300 rounded p-2 text-sm focus:ring-2 focus:ring-slate-500 outline-none"/>
                            </div>
                        </div>
                     ))}
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-slate-200">
                    ...
                    <InputGroup label="Academic History & CRE" value={data.education} onChange={(v)=>handleInputChange('education', v)}/>
                </div>
            </div>
                ...
                <div ref={previewRef} style={{ width: '210mm', minHeight: '297mm', backgroundColor: 'white', boxShadow: '0 10px 40px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ padding: '45px 55px', display: 'flex', gap: '50px', flex: 1 }}>
                        ...
                        <div style={{ flex: 1 }}>
                            ...
                            {generatedCvNumber && (
                                <div style={{ marginTop: '50px', padding: '15px', border: '1px dashed #cbd5e1', borderRadius: '8px' }}>
                                    <div style={{ fontSize: '9px', fontWeight: '900', color: '#94a3b8', textTransform: 'uppercase', marginBottom: '5px' }}>System Reliability ID:</div>
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-slate-200">
            <h3 className="text-lg font-black text-slate-900 mb-2 uppercase tracking-tight">Execute Export?</h3>
            <p className="text-sm text-slate-600 mb-6 font-medium leading-relaxed">This process will handshake with the central database to sync your technical record and generate a trackable CV Serial Number.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowReplaceModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold transition-all uppercase text-xs">Abort</button>
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-6 py-2 rounded-lg text-white font-black uppercase shadow-md transition-all active:scale-95 text-xs" style={{ backgroundColor: templateConfig.primaryColor }}>
                {isDownloading ? "Syncing..." : "Confirm & Export"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white rounded-2xl p-8 max-w-sm w-full text-center shadow-2xl border-b-8 border-emerald-500">
            <div className="w-16 h-16 bg-emerald-50 text-emerald-600 rounded-full flex items-center justify-center mx-auto mb-4">
              <CheckCircle2 size={32}/>
            </div>
            <h3 className="text-xl font-black text-slate-900 mb-2 uppercase">Record Synced</h3>
            <p className="text-sm text-slate-500 mb-6 font-medium leading-tight">Technical specifications have been archived. Your System Reliability ID is:</p>
            ...
            <button onClick={() => setShowSuccessModal(false)} className="w-full py-3 rounded-xl text-white font-black uppercase shadow-lg hover:opacity-90 transition-all text-xs" style={{ backgroundColor: templateConfig.primaryColor }}>
