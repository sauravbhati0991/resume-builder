...
export default function MedicalDirectorProTemplate() {
  ...
  const runDownloadProcess = async () => {
    try {
      ...
      // 4. Trigger Download & Success UI
      await worker.save();
      setGeneratedCvNumber(cvNumber);
      setShowSuccessModal(true);
    } catch (err) {
      console.error("Critical error in download flow:", err);
    } finally {
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Generate PDF?</h3>
            <p className="text-sm text-gray-600 mb-6">This will save your progress to the server and create a unique verification link for this resume.</p>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl text-center">
            <h3 className="text-lg font-bold text-green-700 mb-2">CV Generated!</h3>
            <p className="text-sm text-gray-600 mb-6">Verification ID: <strong>{generatedCvNumber}</strong></p>
            <button onClick={() => setShowSuccessModal(false)} className="px-8 py-2 rounded-lg text-white font-medium" style={{ backgroundColor: templateConfig.primaryColor }}>Finish</button>
          </div>
        </div>
      )}
    </div>
