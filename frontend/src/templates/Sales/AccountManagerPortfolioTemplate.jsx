  ...
  const runDownloadProcess = async () => {
    try {
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `${data.lastName}_Portfolio_${cvNumber}.pdf`,
      ...
      console.error("Critical error in Business Template flow:", err);
    } finally {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 shrink-0 w-full z-10">
        <div className="bg-white/80 backdrop-blur-sm border border-gray-200/60 shadow-lg rounded-xl p-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                <button onClick={saveResumeLocal} disabled={isSaving} className="inline-flex items-center text-sm font-medium h-9 px-4 rounded-md bg-white border border-gray-200 text-gray-700 hover:bg-gray-50 shadow-sm">{isSaving ? <Loader2 className="w-4 h-4 mr-2 animate-spin"/> : <Save className="w-4 h-4 mr-2" />} Save Progress</button>
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Finalize Business Portfolio?</h3>
            <p className="text-sm text-gray-600 mb-6">This will save your current data to our secure server and generate a professional PDF with a unique tracking ID for your applications.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowReplaceModal(false)} className="px-4 py-2 rounded-lg border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors">Adjust More</button>
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-4 py-2 rounded-lg text-white font-medium shadow-md transition-opacity" style={{ backgroundColor: templateConfig.primaryColor }}>
                {isDownloading ? "Generating..." : "Save & Export"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-2">Portfolio Exported</h3>
            <p className="text-sm text-gray-600 mb-4">Your document is ready. Reference this ID for future updates:</p>
            ...
            <button onClick={() => setShowSuccessModal(false)} className="w-full py-2.5 rounded-lg text-white font-bold transition-opacity" style={{ backgroundColor: templateConfig.primaryColor }}>Return to Dashboard</button>
          </div>
        </div>
      )}
    </div>
