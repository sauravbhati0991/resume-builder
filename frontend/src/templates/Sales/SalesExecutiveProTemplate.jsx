...
export default function SalesExecutiveProTemplate() {
  ...
  // MASTER LOGIC: The Handshake & Archive
  const runDownloadProcess = async () => {
    try {
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `SALES_PRO_${cvNumber}.pdf`,
      ...
      console.error("Critical: Sales Sync failed", err);
    } finally {
    ...
    <div className="min-h-screen w-full bg-slate-50 flex flex-col overflow-hidden font-sans text-slate-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 shrink-0 w-full z-10">
        <div className="bg-white border border-slate-200 shadow-md rounded-xl p-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                <button onClick={saveResume} disabled={isSaving} className="inline-flex items-center text-sm font-bold h-9 px-4 rounded-md bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 shadow-sm disabled:opacity-50">
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-slate-100">
            <h3 className="text-xl font-bold text-slate-900 mb-2 italic uppercase">Lock Portfolio?</h3>
            <p className="text-sm text-slate-600 mb-6 font-medium">This will sync your record to the Sales Executive database and generate a trackable ID for your export.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowReplaceModal(false)} className="px-4 py-2 rounded-lg border border-slate-300 text-slate-700 hover:bg-slate-50 font-bold">Cancel</button>
