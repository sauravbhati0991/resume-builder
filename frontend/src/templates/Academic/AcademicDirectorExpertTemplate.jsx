...
export default function AcademicDirectorExpertTemplate() {
  ...
  const saveResume = () => {
    setIsSaving(true);
    setTimeout(() => {
      localStorage.setItem(`academic_draft_${data.lastName}`, JSON.stringify(data));
      setIsSaving(false);
    }, 1000);
  };
  ...
  const runDownloadProcess = async () => {
    try {
      ...
      const res = await api.post("/resumes", {
        ...
        categoryName: "Education & Academia",
        resumeData: data
      });
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `Academic_Director_${data.lastName}_${cvNumber}.pdf`,
    ...
    <div className="min-h-screen w-full bg-gray-50 flex flex-col overflow-hidden font-sans text-slate-800">
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border-l-8 border-[#312e81]">
            <h3 className="text-xl font-bold text-gray-900 mb-2">Finalize Academic Dossier?</h3>
            <p className="text-sm text-gray-600 mb-6 italic font-medium">This will sync your administrative record with the institutional registry and generate a validated PDF document.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowReplaceModal(false)} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors text-xs font-bold uppercase tracking-widest">Cancel</button>
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-4 py-2 rounded-lg text-white font-bold transition-all shadow-md text-xs uppercase tracking-widest" style={{ backgroundColor: templateConfig.primaryColor }}>
