...
export default function SupplyChainDirectorEliteTemplate() {
  ...
  const runDownloadProcess = async () => {
    try {
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `Executive_SCM_${data.lastName}.pdf`,
      ...
      console.error("Critical: Supply Chain Archive Sync failed", err);
    } finally {
  ...
  return (
    <div className="min-h-screen w-full bg-gray-50 flex flex-col overflow-hidden font-sans text-slate-800">
        ...
        <div className="grid lg:grid-cols-2 gap-6 h-full">
            ...
            <div className="h-full overflow-y-auto pr-2 custom-scrollbar pb-20 space-y-6">
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                    ...
                    <textarea rows={4} value={data.summary} onChange={(e)=>handleInputChange('summary', e.target.value)} className="w-full border rounded-md p-3 text-sm outline-none focus:ring-2 focus:ring-[#808000] bg-gray-50/50"/>
                </div>
                ...
                <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-200">
                    ...
                    <InputGroup label="Strategic Skills (Comma separated)" value={data.skills} onChange={(v)=>handleInputChange('skills', v)}/>
                ...
                <div ref={previewRef} style={{ width: '210mm', minHeight: '297mm', backgroundColor: 'white', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', display: 'flex', flexDirection: 'column', position: 'relative', fontFamily: '"Times New Roman", Times, serif' }}>
                    ...
                    {generatedCvNumber && (
                      <div style={{ position: 'absolute', bottom: '15px', right: '20px', fontSize: '9px', color: '#9CA3AF', fontFamily: 'monospace', pointerEvents: 'none' }}>
                        MANIFEST ID: {generatedCvNumber} • SCM-ELITE-PROTOCOL
                      </div>
                    )}
                </div>
            </div>
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-100">
            <h3 className="text-xl font-black text-gray-900 mb-2 uppercase tracking-tight">Sync Executive Asset?</h3>
            <p className="text-sm text-gray-600 mb-6 italic">Proceeding will archive this director-level profile in the secure supply chain repository and generate the verified PDF document.</p>
            <div className="flex justify-end gap-3">
              ...
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-5 py-2 rounded-lg text-white font-black transition-all shadow-md text-xs uppercase" style={{ backgroundColor: templateConfig.primaryColor }}>
                {isDownloading ? "Archiving..." : "Approve Sync"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl text-center border-t-8 border-[#808000]">
            <h3 className="text-xl font-black text-gray-900 mb-2 uppercase tracking-tighter">Archive Confirmed</h3>
            <p className="text-sm text-gray-600 mb-4 font-medium italic">Director profile successfully validated and synced with the logistics network.</p>
            <div className="bg-slate-50 border-2 border-dashed border-gray-200 p-5 rounded-xl mb-6">
               <p className="text-[10px] text-gray-400 uppercase font-black tracking-[4px] mb-1 text-center">Protocol Reference</p>
               <p className="text-3xl font-black text-[#808000] font-mono tracking-tighter">{generatedCvNumber}</p>
            </div>
            <button onClick={() => setShowSuccessModal(false)} className="w-full py-3.5 rounded-lg text-white font-black uppercase tracking-widest shadow-lg hover:shadow-xl transition-all" style={{ backgroundColor: templateConfig.primaryColor }}>Dismiss Protocol</button>
          </div>
        </div>
      )}
    </div>
