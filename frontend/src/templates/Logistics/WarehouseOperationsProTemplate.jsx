  ...
  const runDownloadProcess = async () => {
    try {
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `Warehouse_Pro_${data.lastName}.pdf`,
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 mb-2">Initialize Archive Sync?</h3>
            <p className="text-sm text-gray-600 mb-6">This will sync your operations profile to our secure registry and generate your official documentation.</p>
            <div className="flex justify-end gap-3">
              ...
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-4 py-2 rounded-lg text-white font-semibold transition-all" style={{ backgroundColor: templateConfig.primaryColor }}>
                {isDownloading ? "Processing..." : "Sync & Download"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl text-center border-t-8 border-[#808000]">
            <h3 className="text-xl font-bold text-gray-900 mb-2">Sync Complete</h3>
            <p className="text-sm text-gray-600 mb-4 font-medium italic">Record successfully archived in the Logistics Database.</p>
            <div className="bg-slate-50 border-2 border-dashed border-gray-200 p-4 rounded-xl mb-6 flex flex-col items-center">
               <Box className="text-[#808000] mb-2" size={24}/>
               <p className="text-[10px] text-gray-400 uppercase font-bold tracking-[3px] mb-1">Archive ID</p>
               <p className="text-2xl font-black text-[#808000] font-mono">{generatedCvNumber}</p>
            </div>
            <button onClick={() => setShowSuccessModal(false)} className="w-full py-3 rounded-lg text-white font-bold transition-all shadow-md" style={{ backgroundColor: templateConfig.primaryColor }}>Dismiss</button>
          </div>
        </div>
      )}
    </div>
