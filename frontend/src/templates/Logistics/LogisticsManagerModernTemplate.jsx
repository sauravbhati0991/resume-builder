  ...
  const runDownloadProcess = async () => {
    try {
      ...
      const worker = html2pdf()
        .set({
          margin: 0,
          filename: `Logistics_${data.lastName}_${cvNumber}.pdf`,
      ...
      console.error("Critical: Logistics Archiving failed", err);
    } finally {
        ...
        <div className="bg-white/80 backdrop-blur-sm border border-gray-200/60 shadow-lg rounded-xl p-4 flex justify-between items-center">
            ...
            <div className="flex items-center gap-2">
                ...
                <button onClick={downloadPDF} disabled={isDownloading} className="inline-flex items-center text-sm font-medium h-9 px-4 rounded-md text-white shadow-md hover:opacity-90 transition-all" style={{ backgroundColor: templateConfig.primaryColor }}>
                   {isDownloading ? <><Loader2 className="w-4 h-4 mr-2 animate-spin" /> Archiving...</> : <><Download className="w-4 h-4 mr-2" /> Export PDF</>}
                </button>
            </div>
        </div>
      </div>
      ...
      {showReplaceModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-2xl border border-gray-100">
            <h3 className="text-lg font-bold text-gray-900 mb-2 font-mono">Archive Logistics Record?</h3>
            <p className="text-sm text-gray-600 mb-6 italic">Proceeding will sync this transportation profile with the master logistics database and generate a secure PDF document.</p>
            <div className="flex justify-end gap-3">
              <button onClick={() => setShowReplaceModal(false)} className="px-4 py-2 rounded-lg border border-gray-300 text-gray-700 hover:bg-gray-50 transition-colors text-sm font-bold">CANCEL</button>
              <button onClick={runDownloadProcess} disabled={isDownloading} className="px-4 py-2 rounded-lg text-white font-bold transition-all shadow-md text-sm" style={{ backgroundColor: templateConfig.primaryColor }}>
